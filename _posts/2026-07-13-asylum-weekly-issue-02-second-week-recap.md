---
layout: post
title: Asylum Weekly Issue 02
issue: 02
deck: Command Changes and Cartel Reboots
summary: Fried Rice climbs to Admin Captain, Junke turns up on NSF footage, William nukes his bankroll, SWAT gets new commanders, and NSF fills with former Crimson names.
show_hero: false
editorial_layout: true
description: Asylum Weekly Issue 02 covering APD leadership shifts, NSF footage, William's gambling collapse, and the latest faction reshuffles on Asylum Redux.
---

<div class="issue-package">
  <section class="issue-package__lead">
    <div class="issue-package__kicker"><span>Issue 02</span><span>Altis Dispatch</span></div>
    <h2>Power, policy, and fish sales.</h2>
    <p>The second week of Redux brought APD promotions, NSF sightings, reckless gambling, a SWAT reboot, and the usual struggle to tell menace from washed roleplay.</p>
  </section>

  <section class="issue-story issue-story--feature">
    <div class="issue-story__label section-label">Article 01 · Lead Story</div>
    <h3>The Dragon of Altis</h3>
    <p class="issue-story__standfirst">Fried Rice climbs to the top of the APD ladder as command staff reshuffles around him.</p>
    <p>Fried Rice, also known as Chow Mein and a former member of the infamous Dragons gang, has risen to the pinnacle of the Altis Police Department's ranking structure as the official "Admin Captain", a role formerly known as Chief of Police. Fried Rice has a well-chronicled history in the APD and Asylum in general, having polished his kneepads on DarkKnight since at least 2016.</p>
    <p>In addition, former Captain Bilbo Baggins has resumed his post and filled out the roster of active Captains alongside Rocketneo and Fried Rice. Although many remember Bilbo Baggins as a bureaucratic leech sucking the will to live out of the APD's general populace, recent reports indicate that he has discovered an aspect of humanity and is actually quite amicable.</p>
    <p>Although Asylum Weekly reporters were denied access to the "Back Back Office", our sources report that the Captains have spent the last few days of the week embroiled in policy meetings attempting to assuage the concerns of many community members that the power creep on the cop faction has gone too far.</p>
  </section>

  <section class="issue-story issue-story--media-right">
    <div class="issue-story__label section-label">Article 02 · Video Desk</div>
    <h3>Junke: Friend or Foe?</h3>
    <figure class="issue-story__media issue-story__media--video">
      <div class="video-embed">
        <iframe
          src="https://www.youtube.com/embed/UEeLmt2xlLM"
          title="Junke and Shawn playing Domination with NSF"
          loading="lazy"
          allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
          referrerpolicy="strict-origin-when-cross-origin"
          allowfullscreen>
        </iframe>
      </div>
      <figcaption class="caption">Footage of Junke and Shawn appearing with NSF during Domination. <a href="https://www.youtube.com/watch?v=UEeLmt2xlLM&feature=youtu.be">Watch on YouTube</a></figcaption>
    </figure>
    <p>Asylum Weekly has discovered footage of well-known community members Junke and Shawn playing the Asylum Domination event. Both members appear to have joined the formerly formidable gang NSF, much to the dismay of the gang's leader.</p>
    <p>In this video, we clearly see an example of why Junke should not be allowed to hold a mouse, as he shamelessly whiffs nearly every shot on his own gangmate. The tape raises difficult questions for viewers everywhere: is Junke a trusted ally, or simply another threat to whichever roster is unfortunate enough to accept him?</p>
  </section>

  <section class="issue-story issue-story--brief">
    <div class="issue-story__label section-label">Article 03 · Market Watch</div>
    <h3>William, many pounds, few dollars.</h3>
    <p>A well-known and well-sized community member, William, has recently fallen into the degenerate trap known as gambling. Just moments after one of the server's latest patches went live, he lost a $9.5 million coin flip to one of the server's top players, tachophobia.</p>
    <p>Reeling from this loss, he proceeded to gamble away the remains of his considerable fortune. Asylum Weekly sources report he went from a staggering $22 million net worth to a mere $900,000. One could ponder how a man of his size managed to fall through such small slots.</p>
  </section>

  <section class="issue-story issue-story--brief">
    <div class="issue-story__label section-label">Article 04 · APD Desk</div>
    <h3>SWAT, Special Weapons and Tom.</h3>
    <p>APD Lieutenant Junke and APD Sergeant Tom have been named SWAT Commanders. Asylum Weekly sources report that Junke certainly meets the definition of special, but it remains to be seen whether SWAT can be transformed from its formerly cosmetic role into something more useful.</p>
    <p>If you're interested in applying to join this particular short bus, we at the Asylum Weekly encourage you to reach out to either SWAT Commander and wish you luck in acquiring a slot.</p>
  </section>

  <section class="issue-story issue-story--media-left">
    <div class="issue-story__label section-label">Article 05 · Gang Desk</div>
    <h3>NSF, Trash or Terror?</h3>
    <figure class="issue-story__media">
      <a href="{{ '/assets/newsletters/asylum-weekly-issue-02-nsf.png' | relative_url }}">
        <img src="{{ '/assets/newsletters/asylum-weekly-issue-02-nsf.png' | relative_url }}" alt="NSF members in shallow water beside a red truck">
      </a>
      <figcaption class="caption">NSF members and their latest shoreline activity. <a href="{{ '/assets/newsletters/asylum-weekly-issue-02-nsf.png' | relative_url }}">View full image</a></figcaption>
    </figure>
    <p>Asylum Gaming is not the only thing getting rebooted. NSF gang leader Bera has graciously allowed homeless and washed former Crimson members Junke, Carter, Cudi, Tachophobia, and Shawn to take up residence in the NSF gang.</p>
    <p>These roleplayers can often be spotted on the coast, searching for fish to sell back to the market. With such a nightmarish roster, it is a wonder they can even cap a cartel, much less achieve a coveted Domination win.</p>
  </section>
</div>
